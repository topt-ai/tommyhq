...
export function ContactES() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('setContactModeES', handleSetMode);
    return () => window.removeEventListener('setContactModeES', handleSetMode);
  }, []);
  ...
  return (
    <section id="contacto" className="py-32 px-6 max-w-3xl mx-auto">
      <div className="text-center mb-16">
        <p className="font-mono text-sm tracking-widest uppercase text-brand-accent mb-4">Trabajemos Juntos</p>
        <h2 className="font-serif text-4xl md:text-6xl italic">¿Listo para construir un sistema que funcione solo?</h2>
      </div>
      ...
      <div className="bg-[#0d0d0d] border border-brand-border rounded-3xl p-8 md:p-12">
        ...
        {status === 'success' ? (
          <div className="text-center py-12">
            <h3 className="font-serif text-3xl mb-4 text-brand-accent">Gracias, nos ponemos en ello.</h3>
            <p className="text-brand-muted">Te contactaremos lo más pronto posible.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-mono text-brand-muted uppercase tracking-wider">Nombre</label>
              ...
              <div className="space-y-2">
                <label className="text-sm font-mono text-brand-muted uppercase tracking-wider">Apellido</label>
                <input required type="text" className="w-full bg-brand-bg border border-brand-border rounded-xl px-4 py-3 focus:outline-none focus:border-brand-accent transition-colors" />
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-mono text-brand-muted uppercase tracking-wider">Correo electrónico</label>
              ...
              <div className="space-y-2">
                <label className="text-sm font-mono text-brand-muted uppercase tracking-wider">Nombre del negocio</label>
                <input required type="text" className="w-full bg-brand-bg border border-brand-border rounded-xl px-4 py-3 focus:outline-none focus:border-brand-accent transition-colors" />
              </div>
            </div>
            ...
            {!noWebsite && (
              <div className="space-y-2">
                <label className="text-sm font-mono text-brand-muted uppercase tracking-wider">URL del Sitio Web</label>
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="noWebsiteES" className="text-sm text-brand-muted">Aún no tengo un sitio web</label>
            </div>

            {mode === 'audit' && (
              <div className="space-y-2">
                <label className="text-sm font-mono text-brand-muted uppercase tracking-wider">URL de Google Maps</label>
                <input required type="url" className="w-full bg-brand-bg border border-brand-border rounded-xl px-4 py-3 focus:outline-none focus:border-brand-accent transition-colors" placeholder="Enlace a tu Perfil de Negocio en Google" />
              </div>
            )}

            {mode === 'general' && (
              <div className="space-y-2">
                <label className="text-sm font-mono text-brand-muted uppercase tracking-wider">Mensaje</label>
      ...
      <p className="text-center mt-8 text-brand-muted text-sm">
        O envíanos un correo directamente a <a href="mailto:hola@tommyhq.com" className="text-brand-primary underline">hola@tommyhq.com</a>
      </p>
    </section>
  );
}
